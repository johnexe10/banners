...
var Smoke = function(imgName,width,height,position,amount)
{
...
Smoke.prototype.Particle = function(x, y,scope)
{
  ...
  this.size = rr(1,2)*2;
  this.alpha = 0;
  this.maxAlpha = rr(.1,.5);
  this.rotation = rr(0,360);
  this.maxSize = rr(8,12)*2;	
...
Smoke.prototype.draw = function()
{
	  ...
	  {
		  	var randomY = this.position.y;
		  	var randomX =rr(-100,50)+ this.position.x;
		  	var p = new this.Particle(randomX, randomY,this);
...
Smoke.prototype.stop = function(){
    TweenLite.ticker.removeEventListener("tick", this.draw,this)
}